...
// used for the list item and detail card.
  ...
  return L.divIcon({
    ...
    html: `
      <div style="width:${size}px;height:${size}px;transform:translate(-50%,-100%);filter:${glow};transition:filter 150ms ease">
        <svg viewBox="0 0 24 24" width="${size}" height="${size}" fill="${fill}" stroke="#f6f4fa" stroke-width="1.25">
  ...
  return L.divIcon({
    ...
    html: `
      <div style="position:relative;width:20px;height:20px">
        ...
        <div style="position:absolute;inset:0;border-radius:9999px;background:#3b82f6;border:2px solid #f6f4fa;box-shadow:0 1px 3px rgba(0,0,0,.5)"></div>
      </div>`,
    iconSize: [20, 20],
    iconAnchor: [10, 10],
  });
